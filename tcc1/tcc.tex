...
\par
Para garantir maior segurança de tais projetos, um analisador de código pode ser desenvolvido como solução, com foco na detecção de potenciais potenciais vazamentos e 
...
Sendo apontado pela Microsoft, na qual 70\% das falhas corrigidas adereçadas a um CVE em seus softwares tinham como origem vulnerabilidades de memória geradas pelos próprios desenvolvedores, estando presentes e detectáveis em seus respectivos códigos-fonte \cite{MSRC2019}.
\par
...
\begin{itemize}
\item redis, banco de dados de alto desempenho focado no armazenamento de estruturas em memória.
\item curl, ferramenta de transferência de dados em rede com a utilização de URLs em protocolos diversos.
\item sqlite, motor de banco de dados relacional embutido e autônomo, utilizado em aplicações locais e móveis.
\item musl libc, implementação da biblioteca padrão do C, direcionada a API de chamadas de sistema do Linux.
...
representando porções de memória que existem no contexto atual, mas que não serão tocadas ou alteradas pela operação da função.

\par
Graças a capacidade de sintetizar as especificações utilizando a bi-abdução, o \textit{Infer} realiza uma análise composicional, em que cada procedimento é analisado de forma independente de seus chamadores, numa abordagem de baixo para cima (\textit{bottom-up}). O objetivo principal de tal análise é descobrir automaticamente invariantes que descrevem o uso das estruturas de dados do programa, a fim de provar a segurança de ponteiros, sem exigir que o programador escreva especificações manuais. 
...
\par
O \textit{Infer} processa um projeto através de três etapas principais:
\begin{itemize}
...
O desenvolvimento deste trabalho de conclusão de curso esta dividida em duas seções, sendo a primeira, \ref{sec:solucao} a solução proposta, na qual é discorrido o que será realizado para que os objetivos sejam alcançados, e a seção \ref{sec:cronograma} cronograma de trabalho, expandindo como será realizada cada etapa da solução, bem como serão apresentados prazos para cada fase.
